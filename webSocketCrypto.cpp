...
union sha1_container_t {
	...
	char chars[sizeof(unsigned int) * 5 + 1]; ///< to get simple char values from union memory
};
...
{
    union sha1_container_t sha1_container = {0};
...
{
    return encode64(get_sha1(sec_webSocket_key));///just use static funcs declared above
}
